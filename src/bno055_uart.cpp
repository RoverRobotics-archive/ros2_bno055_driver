...
void BNO055UART::read(uint8_t page)
{
  if (page != page_ && read_command_.address != BNO055Register::PAGE_ID) {
    // TODO: Change pages automtically
  ...
  for (size_t this_write, index = 0; index < sizeof(read_command_); index += this_write) {
    this_write = port_.write((const uint8_t *)&read_command_ + index, 1);
    if (!this_write) {
  ...
  if (!port_.read((uint8_t *)&read_response_, 1)) {
    throw serial::SerialException("Timeout reading from device");
  }

  if (read_response_.message_type == BNO055MessageType::RESPONSE_STATUS) {
    if (!port_.read((uint8_t *)&read_response_ + 1, 1)) {
      throw serial::SerialException("Timeout reading from device");
    }

    std::stringstream ss;
    ss << "Register read failed with error code: 0x" << std::hex << std::setw(2) << read_response_.response_status; 
    throw serial::SerialException(ss.str().c_str());
  } else if (read_response_.message_type != BNO055MessageType::READ_RESPONSE) {
    std::stringstream ss;
    ss << "Invalid response from read command: 0x" << std::hex << std::setw(2) << read_response_.message_type; 
    throw serial::SerialException(ss.str().c_str());
  }
  ...
  for (size_t this_read, remaining = read_response_.length; remaining > 0; remaining -= this_read) {
    this_read = port_.read((uint8_t *)&read_response_ + 2, remaining);
    if (!this_read) {
      throw serial::SerialException("Timeout reading from device");
    }
  }
  ...
  if (read_command_.length != read_response_.length) {
    std::stringstream ss;
    ss << "Requested " << read_command_.length << " bytes from device, but got " << read_response_.length;
    throw serial::SerialException(ss.str().c_str());
  }
}
...
void BNO055UART::write(uint8_t page)
{
  ...
  for (size_t this_write, index = 0; index < write_command_.length + 4u; index += this_write) {
    this_write = port_.write((const uint8_t *)&write_command_ + index, 1);
    if (!this_write) {
      throw serial::SerialException("Timeout writing to device");
    }
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
  ...
  if (!port_.read((uint8_t *)&write_response_, 1)) {
    throw serial::SerialException("Timeout reading from device");
  }

  if (write_response_.message_type != BNO055MessageType::RESPONSE_STATUS) {
    std::stringstream ss;
    ss << "Invalid response from write command: 0x" << std::hex << std::setw(2) << std::setfill('0') << write_response_.message_type;
    throw serial::SerialException(ss.str().c_str());
  }

  if (!port_.read((uint8_t *)&write_response_ + 1, 1)) {
    throw serial::SerialException("Timeout reading from device");
  }

  if (write_response_.response_status != BNO055ResponseStatus::WRITE_SUCCESS) {
    std::stringstream ss;
    ss << "Register write failed with error code: 0x" << std::hex << std::setw(2) << std::setfill('0') << write_response_.response_status; 
    throw serial::SerialException(ss.str().c_str());
  }
}
